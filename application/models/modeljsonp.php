...
class modelJsonp extends CI_Model{
	...
	public function getTable($status,$agency,$company){
		...
		foreach ($rResult->result_array() as $aRow)
		{
            ...
            foreach($nColumns as $col)
            {
                if($col === 'appid'){ 
					$id =  $aRow[$col];
					$row[] = "<input type='checkbox' style='margin-top:0px;'>";
				}
	...
	//table recuited
	public function getTableRecruited($status,$agency,$company){
	...
	public function getTableProccesed($status,$agency,$company){
		...
		if($agency!=null){        
			$sTable = 'applicant LEFT JOIN list ON applicant.position1=list.id';
			$sTable.= '          LEFT JOIN employer ON applicant.emrid=employer.id';
			$sTable.= '          LEFT JOIN documents ON applicant.appid=documents.appid';
        }else{
			$sTable = 'applicant LEFT JOIN list ON applicant.position1=list.id';
		}
	...
	//table recuited
	public function getTableDeparted($status,$agency,$company){
		...
		foreach ($rResult->result_array() as $aRow)
		{
            ...
            if($aRow['complete'])
				$row[] = "<button type=\"button\" class='btn btn-success' \" style=\"width:200px\" onclick=\"window.location.href='".base_url("transaction/departure/".$agency."/view/".$id)."'\"> Complete </button></td>";
			else
				$row[] = "<button type=\"button\" class='btn btn-danger' \" style=\"width:200px\" onclick=\"window.location.href='".base_url("transaction/departure/".$agency."/view/".$id)."'\"> Uncomplete </button></td>";
			$row[] = "<button href=\"#myModal1\" data-toggle=\"modal\"  class=\"btn btn-primary\" onclick=\"approve('{$id}','{$fname} {$lname}')\"><i class='icon-ok icon-white'></i></button></td>";
			$row[] = "<button href=\"#myModal\" data-toggle=\"modal\" class='btn btn-danger' onclick=\"erase('{$id}','{$fname} {$lname}')\"><i class='icon-remove icon-white'></i></button></td>";
